...
class AMPQMMM(nn.Module):
    def __init__(self, activation=nn.SiLU(), **kwargs):
        ...
        self.weights_qmmm = ff_module(self.node_size // 2, 1, output_size=2 * self.n_channels, activation=self.activation) 
        #self.weights_qmmm = ff_module(self.node_size // 4, 1, output_size=2 * self.n_channels, activation=self.activation) 
        ...
        self.edge_layers = nn.ModuleList([ff_module(self.node_size, 1, output_size=19 * self.n_channels, activation=self.activation) for _ in range(self.n_steps)]) 
        self.eq_message_layers = nn.ModuleList([ff_module(self.node_size, 2, output_size=(self.order + 1) * self.n_channels, activation=self.activation) for _ in range(self.n_steps)])
        ...
        self.QM_alpha = ff_module(self.node_size, 2, output_size=self.order * self.n_channels, activation=self.activation, final_activation=nn.Softplus()) 
        self.B_coefficients = ff_module(self.node_size // 2, 1, output_size=self.order, activation=self.activation)
        #self.B_coefficients = ff_module(self.node_size // 4, 1, output_size=self.order, activation=self.activation)
        if self.aniso_esp:
            self.QM_coefficients = ff_module(self.node_size, 2, output_size=self.order + 1, activation=self.activation)
            self.QM_alpha_esp = ff_module(self.node_size, 2, output_size=self.order, activation=self.activation, final_activation=nn.Softplus()) 
            self.B_coefficients_esp = ff_module(self.node_size // 2, 1, output_size=self.order, activation=self.activation) 
            #self.B_coefficients_esp = ff_module(self.node_size // 4, 1, output_size=self.order, activation=self.activation) 
    ...
    def _calculate_coulomb_term(self, graph: Dict[str, torch.Tensor]):
        ...
        if self.aniso_esp:
            ...
            graph['monos'] = QM_charges # We want to use the previously predicted (charge conserving) monopoles.
            coulomb_term = (QM_charges[graph['receivers_esp']] * graph['mm_monos_esp']) / graph['R1_esp']
    ...
    def _embed(self, graph: Dict[str, torch.Tensor]):
        features_edge = torch.cat((graph['edges'], graph['nodes'][graph['receivers']], graph['nodes'][graph['senders']]), dim=-1)
        ...
        graph['weights_qmmm'] = self.weights_qmmm(torch.cat((graph['nodes'][graph['receivers_qmmm']], graph['edges_qmmm']), dim=-1)) 
        return graph
    
    def _pass_messages(self, graph: Dict[str, torch.Tensor]):
        for step, (eq_message_layer, in_message_layer, in_update_layer, edge_layer) in \
            enumerate(zip(self.eq_message_layers, self.in_message_layers, self.in_update_layers, self.edge_layers)): 
            ...
            multi_feature = build_multi_feature(graph) 
            aniso_feature = build_aniso_feature(graph)
            graph['edge_features'] = torch.cat((edge_layer(edge_features) * aniso_feature, graph['edges']), dim=-1)
            ...
            graph['nodes'] = graph['nodes'] + in_update_layer(torch.cat((graph['nodes'], messages, multi_feature), dim=-1)) 
        return graph
        
    def _build_multipoles_esp(self, graph: Dict[str, torch.Tensor]):
        QMMM_edge_features = QMMM_G_matrices(graph) * graph['weights_qmmm'] 
        graph['alphas_esp'] = self.QM_alpha_esp(graph['nodes'])
...
def G_matrices_2(graph: Dict[str, torch.Tensor]):
    ...
    return torch.cat((monos_1, monos_2, monos_1 * monos_2, dipo_mono, mono_dipo, dipo_dipo, D1_Rx1, D2_Rx1, dipo_R, #remove mono-x terms?
                      ...
                      quad_dipo_Rx2, dipo_quad_Rx2), dim=-1).reshape([monos_1.shape[0], -1]) # 
